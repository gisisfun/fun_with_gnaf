...
#library(data.table)
getwd()
setwd("~/Documents/fun_with_gnaf/csv")
#gnaf_df <- csvread("gnaf_feb_2020_address_view.csv",coltypes=c(sprintf("string",1:14), "integer",sprintf("string",1:5),
#                                                              "integer","string",  sprintf("double",1:2),
#                                                              "string"),header = TRUE, na.strings = c(""))
...
#gnaf_df <- fread("gnaf_feb_2020_address_view.csv")
str(gnaf_df)
#gnaf_df[,'AddressText']=trimws(gnaf_df[,'AddressText'])
#gsub("",NA,gnaf_df$Building_Name) -> gnaf_df$Building_Name
#gsub("",NA,gnaf_df$Lot_Number_Suffix) -> gnaf_df$Lot_Number_Suffix
#gsub("",NA,gnaf_df$Lot_Number_Prefix) -> gnaf_df$Lot_Number_Prefix
#gsub("",NA,gnaf_df$Lot_Number) -> gnaf_df$Lot_Number
#gsub("",NA,gnaf_df$Flat_Number_Suffix) -> gnaf_df$Flat_Number_Suffix
#gsub("",NA,gnaf_df$Flat_Number_Prefix) -> gnaf_df$Flat_Number_Prefix
#gsub("",NA,gnaf_df$Flat_Number) -> gnaf_df$Flat_Number
#gsub("",NA,gnaf_df$Number_First_Prefix) -> gnaf_df$Number_First_Prefix
#gsub("",NA,gnaf_df$Number_First) -> gnaf_df$Number_First
#gsub("",NA,gnaf_df$Level_Number) -> gnaf_df$Level_Number
...
find_address <- function(searchText,gnaf_df,id=1) {
  ...
  #search_result <- gnaf_df[like(AddressText,searchText)][1]
