...
for index,row in df.iterrows():
    v_stmt_loop = v_stmt.where(
            and_(v_address_view.columns.Longitude.between(int(row.longitude)-1,
                                                          int(row.longitude)+1),
    v_address_view.columns.Latitude.between(int(row.latitude)-1,
                                            int(row.latitude)+1)))
    for result in connection.execute(v_stmt_loop):
        ...
        if distance_km < 50:
            print(result.Address_Detail_PID,result.AddressText,
                  result.Latitude, result.Longitude,row.locality_pid,
                  row.locality_name,row.latitude, row.longitude, 
                  distance_km)
            closest.append([result.Address_Detail_PID,result.AddressText,
                            result.Latitude,result.Longitude, row.locality_pid,
                            row.locality_name,row.latitude, row.longitude,
                            distance_km])
